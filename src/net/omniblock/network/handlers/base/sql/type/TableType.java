...
public enum TableType {
	...
	BETAKEYS("betakeys", "CREATE TABLE IF NOT EXISTS betakeys (id INTEGER PRIMARY KEY AUTO_INCREMENT, p_id TEXT(20) NOT NULL, p_key TEXT(10) NOT NULL )", "INSERT INTO betakeys (p_id, p_key) SELECT * FROM (SELECT VAR_P_ID a, VAR_P_KEY b) AS tmp WHERE NOT EXISTS (SELECT 1 FROM betakeys WHERE p_id = p_id);", false, false),
	PLAYERWARPS("playerwarps", "CREATE TABLE IF NOT EXISTS playerwarps (id INTEGER PRIMARY KEY AUTO_INCREMENT, p_id TEXT(20) NOT NULL, p_warps TEXT(10) NOT NULL )", "INSERT INTO playerwarps (p_id, p_warps) SELECT * FROM (SELECT VAR_P_ID a, VAR_P_WARPS b) AS tmp WHERE NOT EXISTS (SELECT 1 FROM playerwarps WHERE p_id = p_id);", false, false),
