package net.omniblock.network.library.helpers.bossbar;
...
import net.omniblock.network.library.helpers.bossbar.nms.FakeDragon;

import java.util.HashMap;
import java.util.UUID;

/**
 * Allows OmniNetwork.getPlugin()s to safely set a health bar message.
 *
...
public class BarAPI extends JavaPlugin implements Listener {
	 ...
	 * It will remain there until the player logs off or another
	 * OmniNetwork.getPlugin() overrides it.<br>
	...
	public static void setMessage(final Player player, String message, int seconds) {
		...
		timers.put(player.getUniqueId(), Bukkit.getScheduler().runTaskTimer(OmniNetwork.getInstance(), new Runnable() {
	...
	private static Location getDragonLocation(Location loc) {
		...
		if (pitch >= 55) {
			...
			loc = loc.getBlock()
					.getRelative(getDirection(loc), OmniNetwork.getInstance().getServer().getViewDistance() * 16)
					.getLocation();
		}
	...
	public void onDisable() {
		for (Player player : OmniNetwork.getInstance().getServer().getOnlinePlayers()) {
			quit(player);
		}
	...
	private void handleTeleport(final Player player, final Location loc) {
		...
		Bukkit.getScheduler().runTaskLater(OmniNetwork.getInstance(), new Runnable() {
